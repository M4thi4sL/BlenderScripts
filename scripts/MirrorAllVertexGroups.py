...
def copy_mirror_weight(
    Axis, Way, Pattern, special_pattern, left_side, right_side, tolerance, ignore_locked
):
    ...
    for vertex in bpy.data.meshes[mesh].vertices:
        ...
        if (vertex.co[axis_num] > tolerance and Way == "normal") or (
            vertex.co[axis_num] < -tolerance and Way == "reverse"
        ):  # compare with the tolerance and position of the origin
    ...
    for vertex in DictVertexOri:
        ...
        for groups in (
            bpy.data.meshes[mesh].vertices[vertex].groups
        ):  # when it's the automatic pattern, it checks the most used on the original side .L or .R and assign them after
        ...
        for (
            ...
            DictVertexDest
        ):  # check the coordinates of each axis, the multiply is inversing the mirror axes from -x.xxxx to x.xxxx so that the side doesn't matter
            if (
                vertexCoXOri - tolerance
                < DictVertexDest[vertexMirror][0] * x_multiply
                < vertexCoXOri + tolerance
            ):
                if (
                    vertexCoYOri - tolerance
                    < DictVertexDest[vertexMirror][1] * y_multiply
                    < vertexCoYOri + tolerance
                ):
                    if (
                        vertexCoZOri - tolerance
                        < DictVertexDest[vertexMirror][2] * z_multiply
                        < vertexCoZOri + tolerance
                    ):
    ...
    for vertex in DictMirror:
        ...
        for groups in bpy.data.meshes[mesh].vertices[vertex].groups:
            myGroupVertex[groups.group] = (
                groups.weight
            )  # create a dictionary nameGroup = weight

            for (
                OneGroup
            ) in myGroupVertex:  # create a new dictionnary with the name mirror

                ignored = ignore_locked and obj.vertex_groups[OneGroup].lock_weight

                if not ignored:
                    if obj.vertex_groups[OneGroup].name.find(ori_side) > -1:
                        mirror_normal_group_name = obj.vertex_groups[
                            OneGroup
                        ].name.replace(ori_side, dest_side)
                        if mirror_normal_group_name in DictGroupINV:
                            myMirrorGroupVertex[
                                DictGroupINV[mirror_normal_group_name]
                            ] = myGroupVertex[OneGroup]
                        else:
                    ...
                    if obj.vertex_groups[OneGroup].name.find(dest_side) > -1:
                        mirror_normal_group_name = obj.vertex_groups[
                            OneGroup
                        ].name.replace(dest_side, ori_side)
                        if mirror_normal_group_name in DictGroupINV:
                            myMirrorGroupVertex[
                                DictGroupINV[mirror_normal_group_name]
                            ] = myGroupVertex[OneGroup]
                    ...
                    if (
                        obj.vertex_groups[OneGroup].name.find(ori_side) == -1
                        and obj.vertex_groups[OneGroup].name.find(dest_side) == -1
                    ):
        ...
        for group in bpy.data.meshes[mesh].vertices[DictMirror[vertex]].groups:
            ...
            ignored = (
                ignore_locked and obj.vertex_groups[DictGroup[group.group]].lock_weight
            )
            if not ignored:
        ...
        for OneGroupVertex in myMirrorGroupVertex.keys():
            ...
            if not ignored:
                obj.vertex_groups[DictGroup[OneGroupVertex]].add(
                    [DictMirror[vertex]], myMirrorGroupVertex[OneGroupVertex], "REPLACE"
                )
...
class BR_OT_mirror_all_vertex_groups(bpy.types.Operator):
    ...
    tolerance: FloatProperty(
        name="Tolerance", min=0, max=100, precision=3, default=0.001
    )
...
def menu_draw(self, context):
    ...
    self.layout.operator(
        BR_OT_mirror_all_vertex_groups.bl_idname,
        text="Mirror all vertex groups",
...
classes = (BR_OT_mirror_all_vertex_groups,)
